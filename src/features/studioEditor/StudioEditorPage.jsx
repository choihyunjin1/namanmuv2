...
import { ChevronDown, ChevronUp, Cuboid, Eye, EyeOff, Layers3, Lock, PanelRightClose, Unlock } from "lucide-react";
import {
  CAMERA_VIEW_OPTIONS,
  EDITOR_FLOORS,
  EDITOR_GRID,
...
const VIEW_DISPLAY_CAMERA_OPTIONS = [
...
function StudioEditorViewDisplayBar({
  cameraView,
  gridVisible,
  onCameraViewChange,
  onGridToggle,
...
function StudioEditorCanvasHud({
  activeCategoryLabel,
  activeFloor,
  activeModeLabel,
  activeToolLabel,
...
function StudioViewportFloorStack({
  activeFloor,
  activeFloorBaseY,
  floors = [],
  onFloorChange,
  ...
  return (
    <aside className="studio-editor-floor-stack" aria-label="층 스택 내비게이터">
      <header className="studio-editor-floor-stack-header">
        ...
        <em>{formatInspectorMeters(activeFloorBaseY)}</em>
      </header>
...
function StudioEmptySceneGuide({ activeFloor, planMode }) {
  return (
    <div
...
function StudioSelectionActionBar({
  activeTool,
  canDelete = false,
  canDuplicate = false,
